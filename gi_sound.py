...
class GiSpeech:
    def __init__(self, a1, a2, a3, a4, a5, a6, sby, rst, ald):
        # Set up GPIO pins.
        ...
        GPIO.setup(self.a3, GPIO.OUT)
        GPIO.setup(self.a4, GPIO.OUT)
        GPIO.setup(self.a5, GPIO.OUT)
        GPIO.setup(self.a6, GPIO.OUT)

        GPIO.setup(self.ald, GPIO.OUT)
        GPIO.setup(self.rst, GPIO.OUT)
        GPIO.setup(self.sby, GPIO.IN)

        # ALD starts high.
        GPIO.output(self.ald, 1)
    ...
    def speak(self, code):
        # Write allophone code to address lines.
        b_str = format(code, '06b')
        GPIO.output(self.a1, int(b_str[5]))
        GPIO.output(self.a2, int(b_str[4]))
        GPIO.output(self.a3, int(b_str[3]))
        GPIO.output(self.a4, int(b_str[2]))
        GPIO.output(self.a5, int(b_str[1]))
        GPIO.output(self.a6, int(b_str[0]))
        
        # Latch data.
        GPIO.output(self.ald, 0)
        time.sleep(0.001)
        GPIO.output(self.ald, 1)

        # Wait until speech is complete.
        while (GPIO.input(self.sby) == 0):
            pass
